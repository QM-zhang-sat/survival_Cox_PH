...
# optimHess is an auxiliary function to compute the Hessian at a later stage if hessian = TRUE was forgotten.
...
index_ini = get_initial_value_parallel(para_list, clayton_loglik, X, Y, d1, d2 ) 
ini_para= combin_para[index_ini[1],]
# c(0.1,0.1,0.4)
clayton_optim_grid <- optim(ini_para, clayton_loglik, method="L-BFGS-B",lower=c(0.01,0.01,0.1),upper=c(0.2,0.2,20), X=df$X, Y=df$Y, d1=df$d1, d2=df$d2, control=list(fnscale=-1), hessian=TRUE)
...
##Frank & Exponential copula analysis##  c(0.1,0.1,0.4)
par2_frank<-par1_frank <- seq(-5 ,5,0.2)   #  0.01,10,1
par3_frank<- seq(-5,5,0.2) #. 0.1,1,0.1
para_list_frank= list(par1_frank, par2_frank, par3_frank)

index_ini_frank = get_initial_value_parallel(para_list_frank, frank_loglik, X, Y, d1, d2 ) 
combin_para_frank= expand.grid(para_list_frank)
ini_para_frank= combin_para_frank[index_ini_frank[1],]
# ini_para_frank = get_initial_value(para_list_frank, frank_loglik, X, Y, d1, d2 ) 
frank_optim_grid <- optim(ini_para_frank, frank_loglik, method="L-BFGS-B",lower=c(0.01,0.01,0.1),upper=c(0.2,0.2,20), X=df$X, Y=df$Y, d1=df$d1, d2=df$d2, control=list(fnscale=-1), hessian=TRUE)
...
index_ini_gumbel = get_initial_value_parallel(para_list_gumbel, gumbel_loglik, X, Y, d1, d2 ) 
combin_para_gumbel = expand.grid(para_list_gumbel)
ini_para_gumbel= combin_para_gumbel[index_ini_gumbel[1],]

gumbel_optim_grid <- optim(ini_para_gumbel, gumbel_loglik, method="L-BFGS-B",lower=c(0.01,0.01,1.01),upper=c(0.2,0.2,20), X=df$X, Y=df$Y, d1=df$d1, d2=df$d2, control=list(fnscale=-1), hessian=TRUE)
...
index_ini = get_initial_value_parallel(para_list, gumbel_loglik, X, Y, d1, d2 ) 
ini_para_normal= combin_para[index_ini[1],]
normal_optim <- optim(ini_para_normal, normal_loglik, method="L-BFGS-B",lower=c(0.01,0.01,0.01),upper=c(0.2,0.2,0.99), X=df$X, Y=df$Y, d1=df$d1, d2=df$d2, control=list(fnscale=-1), hessian=TRUE)
